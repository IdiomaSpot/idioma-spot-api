...
export abstract class GenericController<
  ...
  Service extends GenericService<Entity, ResponseType>,
  ResponseType = Entity,
> {
  ...
  @Get()
  @ApiBearerAuth()
  async find<ReturnType = ResponseType>(
    @Query() options?: FindManyOptions<Entity>,
  ): Promise<ReturnType[]> {
  ...
  @Patch(':id')
  @ApiBearerAuth()
  async update<ReturnType = ResponseType>(
    @Param('id') id: number,
    @Body() entity: Entity,
  ) {
